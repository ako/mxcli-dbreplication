...
import databasereplication.implementation.DBReplicationExecutionSettings.JoinedTable;
 ...
 */
public class ImportByMapping extends UserAction<java.lang.Boolean>
{
	...
	throws MendixReplicationException, CoreException
	{
		...
		{
			String objectOneTypeName = importCall.getImportCallOne_MxObjectType(context).getCompleteName();
			if (objectOne == null) {
				throw new CoreException("The input object one is null but an object of type " + objectOneTypeName + " was expected.");
			} else if (!objectOne.getMetaObject().isSubClassOf(objectOneTypeName))
			{
				throw new CoreException("The input object one is not of type: " + objectOneTypeName + " but is " + objectOne.getType());
			}
		}
		if(importCall.getImportCalTwo_MxObjectType() != null)
		{
			String objectTwoTypeName = importCall.getImportCalTwo_MxObjectType(context).getCompleteName();
			if (objectTwo == null) {
				throw new CoreException("The input object two is null but an object of type " + objectTwoTypeName + " was expected.");
			} else if (!objectTwo.getMetaObject().isSubClassOf(objectTwoTypeName))
			{
				throw new CoreException("The input object two is not of type: " + objectTwoTypeName + " but is " + objectTwo.getType());
			}
		}

		IDataManager manager = InitialiseImport( context, Mapping, dbSettings );
		DBReplicationExecutionSettings settings = manager.getSettings();
		...
		String constraintsQuery = "//"+CustomConstraint.getType()+"["+CustomConstraint.MemberNames.CustomConstraint_ImportCall.toString()+"="+importCall.getMendixObject().getId().toLong()+"]";
		List<IMendixObject> constraints = Core.createXPathQuery(constraintsQuery).execute(context);
	...
	public static IDataManager InitialiseImport(IContext context, TableMapping Mapping, IDatabaseSettings dbSettings ) throws MendixReplicationException, CoreException {
		DBReplicationExecutionSettings
				settings = new DBReplicationExecutionSettings(context, dbSettings, Mapping.getTableMapping_MxObjectType(context).getCompleteName());
		...
		settings.setMappingTimeZoneWithPriority(Mapping.getTableMapping_TimeZone(), dbSettings.getDatabaseTimezone());
		...
		List<IMendixObject> joinTables = Core.createXPathQuery(xpathQuery).execute(context);
		 ...
		 */
		String colMappingQuery = "//" + ColumnMapping.getType() + "[" + ColumnMapping.MemberNames.ColumnMapping_TableMapping + "=" + Mapping.getMendixObject().getId().toLong() + "]";
		List<IMendixObject> colMapping = Core.createXPathQuery(colMappingQuery).execute(context);
		 ...
		 */
		String refHandlingQuery = "//" + ReferenceHandling.getType() + "[" + ReferenceHandling.MemberNames.ReferenceHandling_TableMapping + "=" + Mapping.getMendixObject().getId().toLong() + "]";
		List<IMendixObject> refHandlingList = Core.createXPathQuery(refHandlingQuery).execute(context);
...
//		settings.Configuration.MetaInfoProcessingBatchThreshold = 10; //default 1000
	...
	private static void processJoinedTable(IContext context, TableMapping Mapping, DBReplicationExecutionSettings settings,
																				 IMendixObject additionalJoinObject, int joinLevel, ArrayList<String> processedTables)
		...
		if( leftId != null ) {
			...
			if( joinType == JoinType.FROM ) {
				...
				String constraintsQuery = "//" + Constraint.getType() + "[" + Constraint.MemberNames.Constraint_AdditionalJoins + "=" + additionalJoinObject.getId().toLong() + "]";
				List<IMendixObject> constraints = Core.createXPathQuery(constraintsQuery).execute(context);
		...
		String joinTablesQuery = "//" + AdditionalJoins.getType() + "[" + AdditionalJoins.MemberNames.AdditionalJoins_TableMapping + "=" + Mapping.getMendixObject().getId().toLong() + "][" + AdditionalJoins.MemberNames.AdditionalJoins_AdditionalJoins_Left + "=" + additionalJoinObject.getId().toLong() + "]";
		List<IMendixObject> joinTables = Core.createXPathQuery(joinTablesQuery).execute(context);
	...
	private static String processStatement( IContext context, IDatabaseConnector dbType, TableMapping Mapping, String statement, String fromAlias ) throws CoreException {
		String joinsQuery = "//" + AdditionalJoins.getType()+ "[" + AdditionalJoins.MemberNames.AdditionalJoins_TableMapping + "=" + Mapping.getMendixObject().getId().toLong() + "]";
		List<IMendixObject> joins = Core.createXPathQuery(joinsQuery).execute(context);
	...
	private static String getObjectValue(IContext context, IMendixObject objectOne, IMendixObject objectTwo, CustomConstraint custom) throws CoreException
	{
		...
		{
                    case Object1:
						return parseValueToString(objectOne.getMember(member.getAttributeName()), context);
                    case Object2:
						return parseValueToString(objectTwo.getMember(member.getAttributeName()), context);
		}
