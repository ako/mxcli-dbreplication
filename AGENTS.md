# Mendix Project: mxcli-dbreplication
...
```bash
./mxcli -p ReplicationLab.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p ReplicationLab.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check ReplicationLab.mpr   # Validate project
./mxcli docker check -p ReplicationLab.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p ReplicationLab.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p ReplicationLab.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p ReplicationLab.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p ReplicationLab.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p ReplicationLab.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'ReplicationLab.mpr';
```
...
./mxcli check script.mdl -p ReplicationLab.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p ReplicationLab.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p ReplicationLab.mpr

# With colored output
./mxcli lint -p ReplicationLab.mpr --color

# List available rules
./mxcli lint -p ReplicationLab.mpr --list-rules

# Output as SARIF
./mxcli lint -p ReplicationLab.mpr --format sarif > results.sarif
```
...
./mxcli report -p ReplicationLab.mpr

# JSON report
./mxcli report -p ReplicationLab.mpr --format json

# HTML report
./mxcli report -p ReplicationLab.mpr --format html
```
